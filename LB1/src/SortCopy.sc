...
(procedure (SortedAdd theOrigin theLists 
	...
	(repeat
		...
		(= distMinFront	(= distMinOut	(= distMinBack	$7fff)))
		...
		(for
			...
			(for	
				...
				(if
					(or
						(and
							...
							(& (obj signal?) (| $0080 $0002)) ;evSAID mouseUp))
						)
				...
				(if (= invisible (CantBeSeen obj theOrigin))
					...
					(= dist (Abs (kernel_111 ang dist)))
				)
				
				(if (< dist 0) (= dist $7fff))	;overflow compensation
